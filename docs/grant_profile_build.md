...
- Code: `mapping/canonical_mapper.py`, `mapping/embedding_matcher.py`

3) Metadata Attachment
- Reads and attaches the taxonomy version to the profile.
- Code: `mapping/grant_profile_builder.py`
...
- The profile includes a `deadline` block parsed from the text (heuristic):
  ...
  - Code: `extraction/deadline_extractor.py`
...
- Derive grant id from filename:
  - `python -m mapping.grant_profile_builder data/grants/text_grant_1.txt`
- Or specify the id and output directory:
  - `python -m mapping.grant_profile_builder data/grants/text_grant_1.txt --grant-id text_grant_1 --out-dir data/processed_grants`
- Optionally include a source URL:
  - `python -m mapping.grant_profile_builder data/grants/text_grant_1.txt --source-url https://example.org/rfp`
...
- Or customize directory and output location:
  - `python -m mapping.grant_profile_builder --all --dir data/grants --ext .txt --out-dir data/processed_grants`
