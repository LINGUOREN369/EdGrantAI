...
See also: docs/mapping_funnel.md for the full dictionary→guardrails→embeddings funnel and curation policy.
...
Precedence
- Runtime loader uses curated files only (auto files are ignored).
- The build merges auto into curated beforehand with manual precedence and deletes `*.auto.json`.
...
Deciding dictionary vs embeddings (summary)
- Put into dictionary when: high-precision acronym/shorthand or common domain paraphrase (e.g., REU, LEAs, IHE, EPSCoR; “informal science education”).
- Leave to embeddings when: ambiguous single words, vendor names, or context-dependent phrases.
